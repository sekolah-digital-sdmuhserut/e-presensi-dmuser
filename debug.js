...
const DB_ROOT = ''; 
function rootRef(path) { return db.ref((DB_ROOT ? DB_ROOT + '/' : '') + path); }
...
// --- INITIALIZATION ---
rootRef('pengaturan/logoUrl').on('value', snap => {
    const url = snap.val();
...
function cekKode(destination) {
    ...
    rootRef('database/' + kode).once('value', (snap) => {
        const data = snap.val();
...
function kirimIzin() {
    ...
    rootRef('requests_izin').push(payload).then(() => {
...
function loadInitialData(nama) {
    Promise.all([
        rootRef('pengaturan').once('value'),
        rootRef('requests_izin').orderByChild('nama').equalTo(nama).limitToLast(50).once('value')
        ...
        window.masterTunjanganData = masterTunjangan;
...
// FUNGSI PENDUKUNG (Diletakkan di atas agar tidak error "Unknown identifier")
function activateLogListeners(nama) {
    rootRef('logs_presensi_sekolah').on('value', () => { refreshMainHistory(); });
    rootRef('logs_pengajian').on('value', () => { refreshMainHistory(); });
    rootRef('logs_ekstrakurikuler').on('value', () => { refreshMainHistory(); });
}

function setupPayrollListener() {
    let date = new Date();
    if (date.getDate() > 25) date.setMonth(date.getMonth() + 1);
    const periodStr = date.getFullYear() + '-' + String(date.getMonth() + 1).padStart(2, '0');
    
    rootRef('payroll_published/' + periodStr).on('value', (snap) => {
...
function setupRealtimeListeners(nama) {
    // 1. LISTENER IZIN
    rootRef('requests_izin').orderByChild('nama').equalTo(nama).limitToLast(50).on('value', (snap) => {
        const data = snap.val();
        cachedIzinRequests = [];
        if (data) {
    ...
    // 2. LISTENER PENGATURAN (LOAD DAHULU - FIX PIKET)
    rootRef('pengaturan').once('value').then((snap) => {
        const settingsData = snap.val() || {};
        globalSettings = settingsData.rules || {};
        globalPiket = (settingsData.masterData && settingsData.masterData.piket) ? settingsData.masterData.piket : {};
        currentXtraList = settingsData.xtraList || [];

        // Activate logs AFTER settings are loaded
        activateLogListeners(nama);
        setupPayrollListener();
        
        refreshMainHistory();
    });

    // 3. LISTENER PENGATURAN REALTIME
    rootRef('pengaturan').on('value', (snap) => {
        const settingsData = snap.val() || {};
        globalSettings = settingsData.rules || {};
        globalPiket = (settingsData.masterData && settingsData.masterData.piket) ? settingsData.masterData.piket : {};
        currentXtraList = settingsData.xtraList || [];
...
function refreshMainHistory() {
    ...
    this.refreshTimeout = setTimeout(() => {
        Promise.all([
            rootRef('logs_presensi_sekolah').limitToLast(500).once('value'),
            rootRef('logs_pengajian').limitToLast(500).once('value'),
            rootRef('logs_ekstrakurikuler').limitToLast(500).once('value')
...
function renderIzinTable(requests) {
    ...
    requests.forEach(req => {
        ...
        const row = '<tr>' +
                ...
                '<td style="vertical-align:top; padding-top:16px;"><span class="badge bg-purple" style="font-size:9px; padding:4px 8px;">' + req.jenis + '</span></td>' +
                ...
                Object.values(logsPerDate).forEach(log => {
                    if (log.nama === currentUserData.nama) {
                        ...
                        if (dateObj && !isNaN(dateObj.getTime())) {
                            ...
                            const isPiket = piketList.some(name => name.toLowerCase() === currentUserData.nama.toLowerCase());
...
function bukaModalSlipGaji() {
    ...
    let persenClass = 'persen-low';
    let bgPersen = '#fee2e2';
    let colorPersen = '#991b1b';
    
    if (row.persenHadir >= 90) {
        persenClass = 'persen-high'; 
        bgPersen = '#dcfce7'; 
        colorPersen = '#166534';
    } else if (row.persenHadir >= 70) {
        persenClass = 'persen-mid'; 
        bgPersen = '#fef3c7'; 
        colorPersen = '#92400e';
    }
    ...
    const slipHtml = 
        ...
        '<div style="display:grid; grid-template-columns:1fr 1fr; gap:16px; background:#f8fafc; padding:16px; border-radius:12px; margin-bottom:24px; font-size:13px;">' +
            ...
            '<div><span style="color:#475569;">Kehadiran Aktif</span><br>' +
                ...
                '<span class="persen-badge ' + persenClass + '" style="margin-left:6px; font-size:10px; padding:2px 6px; border-radius:4px; background:' + bgPersen + '; color:' + colorPersen + ';">' + row.persenHadir + '%</span>' +
            '</div>' +
        '</div>' +
